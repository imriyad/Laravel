...
<body class="bg-gray-900 text-white p-6">
    ...
    <input type="text" name="query" placeholder="Search posts..." value="{{ request('query') }}" class="px-4 py-2 rounded text-white" required>
    <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">Search</button>
</form>


        <a class="bg-green-500 p-4 px-8 rounded" href="/create">Create New Post</a>
                ...
                @forelse ($posts as $post)
                    <tr class="border-t border-gray-700 hover:bg-gray-700">
                        ...
                        <td class="py-3 px-6 text-center space-x-2">
                            ...
                            <form action="{{ route('delete', $post->id) }}" method="GET" style="display:inline;" onsubmit="return confirm('Are you sure you want to delete this post?');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="bg-red-500 hover:bg-red-500 text-black py-1 px-3 rounded">Delete</button>
                            </form>
